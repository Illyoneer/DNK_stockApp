...
import ru.glushko.dnkstockapp.data.datautils.ItemMapper
...
class ItemRepositoryImpl(application: Application) : ItemRepository {
    ...
    override suspend fun deleteItem(item: Item) = _itemDao.deleteItem(_mapper.mapEntityToDBItem(item))

    override suspend fun updateItem(item: Item) = _itemDao.updateItem(_mapper.mapEntityToDBItem(item))
